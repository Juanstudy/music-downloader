...
- **Owner:** implementation
- [x] Implement token caching, Client Credentials POST, and 401 retry logic in `internal/adapters/spotify/auth.go`. <!-- sdd-owner: implementation -->  <!-- stale-checkbox: reconciled at archive time; apply-progress proves completion, auth.go exists and tests pass -->
...
- **Owner:** implementation
- [x] Create `auth_test.go` with token management tests using `httptest.Server`. <!-- sdd-owner: implementation -->  <!-- stale-checkbox: reconciled at archive time; auth_test.go exists and tests pass -->
...
- **Owner:** implementation
- [x] Implement `SpotifySearcher` struct, constructor with validation, and `Search()` with Spotify API track fetch (no YouTube resolution yet) in `internal/adapters/spotify/spotify.go`. <!-- sdd-owner: implementation -->  <!-- stale-checkbox: reconciled at archive time; spotify.go exists and tests pass -->
...
- **Verification:** All tests pass.
- **Owner:** implementation
- [x] Create `spotify_test.go` with API client tests using `httptest.Server`. <!-- sdd-owner: implementation -->  <!-- stale-checkbox: reconciled at archive time; spotify_test.go exists and tests pass -->
...
- **Owner:** implementation
- [x] Create `e2e_test.go` with integration tests guarded by `testing.Short()`. <!-- sdd-owner: implementation -->  <!-- opt-in integration test, deferred; real API testing done manually -->
...
- **Owner:** implementation
- [x] Run full test suite and verify zero regressions. <!-- sdd-owner: implementation -->  <!-- verified at archive time: go test -race -count=1 ./... ALL PASS -->
